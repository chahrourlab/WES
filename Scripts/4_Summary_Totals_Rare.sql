	DROP TABLE IF EXISTS Final_Summary_Totals_Rare;
	create table  Final_Summary_Totals_Rare as
	(SELECT rv.sampleID as Sample,
 	   ...
 	   sum(case when (length(Ref) = 1 and Ref <> '-') and (length(Alt) = 1 and Alt <> '-') then 1 else 0 end) as SNVs,
 	 	0 as Indels
 	   FROM Final_Rare_Variants rv 
	group by rv.sampleID);

	update Final_Summary_Totals_Rare set Indels = Variants - SNVs; 